  ...
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-start justify-center leading-[0] not-italic relative shrink-0 text-nowrap w-full">
      <div className="flex flex-col ustify-center relative shrink-0 text-[#151b26] onboarding-big">
        <p className="leading-[1.6] text-nowrap whitespace-pre">보유 중인 개수를 입력해 주세요</p>
      </div>
      <div className="flex flex-col font-['Pretendard:Medium',sans-serif] justify-center relative shrink-0 text-[#a1a4a8] text-[12px] tracking-[0.24px]">
        <p className="leading-[1.5] text-nowrap whitespace-pre">보유 중이지 않은 경우에도 AI의 분석 결과를 받을 수 있어요.</p>
      </div>
    </div>
  );
}
...
function Frame6({ quantity, setQuantity }: { quantity: string; setQuantity: (value: string) => void }) {
  return (
...
function Frame8({ quantity, setQuantity }: { quantity: string; setQuantity: (value: string) => void }) {
  return (
...
function Frame5({ quantity, setQuantity }: { quantity: string; setQuantity: (value: string) => void }) {
  return (
...
export default function StockQuantityInput({ stockName, onBack, onSubmit }: StockQuantityInputProps) {
